...
void jacobi_step_optimized(const std::vector<double>& cur, std::vector<double>& next, int n) {
    // TODO: Add OpenMP parallelism here. Keep boundary values unchanged.
    for (int i = 1; i < n - 1; ++i) {
        for (int j = 1; j < n - 1; ++j) {
            int id = i * n + j;
...
int main(int argc, char** argv) {
    ...
    for (int t = 0; t < steps; ++t) {
        jacobi_step_optimized(cur, next, n);
        cur.swap(next);
    }
