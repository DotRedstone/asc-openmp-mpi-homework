...
void merge_sort_optimized(std::vector<int>& a, std::vector<int>& tmp, std::size_t l, std::size_t r) {
    if (r - l <= 1) return;
    std::size_t m = l + (r - l) / 2;

    // TODO: Replace this serial recursion with an OpenMP task-based version.
    // Hint: set a threshold, and only create tasks for large enough ranges.
    merge_sort_optimized(a, tmp, l, m);
    merge_sort_optimized(a, tmp, m, r);
...
int main(int argc, char** argv) {
    ...
    merge_sort_optimized(a, tmp, 0, a.size());
    ...
    std::cout << "program=merge_sort_omp\n";
    std::cout << "n=" << n << "\n";
    std::cout << "seed=" << seed << "\n";
    std::cout << "threads=" << openmp_threads() << "\n";
